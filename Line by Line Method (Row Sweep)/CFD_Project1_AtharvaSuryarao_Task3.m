...
while error>tolerance
    ...
    for i = 2:N-1
        ...
        A_matrix_row = zeros(N,N);
        B_vector_row = zeros(N,1);
        ...
        % Interior nodes
        for j = 2:N-1
            A_matrix_row(j,j-1) = -1;
            A_matrix_row(j,j)   = 4;
            A_matrix_row(j,j+1) = -1;            
            B_vector_row(j) = T_old(i-1,j) + T_old(i+1,j);
        end
        ...
        %Calculating the temperature of row using TDMA Solver
...
title("Temperature Distribution using the Row Sweep method");
xlabel("X (m)");
ylabel("Y (m)");

...
% function T = Column_Sweep(N_x,N_y)
...
while error>tolerance
    ...
    for i = 2:N_x-1 
        ...
        % Interior nodes
        for j = 2:N_y-1
            A_matrix_column(j,j-1) = -1*delta_x^2;
            A_matrix_column(j,j)   = 2*delta_x^2 + 2*delta_y^2;
            A_matrix_column(j,j+1) = -1*delta_x^2;            
            B_vector_column(j) = (T_old(j,i-1) + T_old(j,i+1))*delta_y^2;
...
xlabel("Iterations");
ylabel("Residual");
title("Convergence of Column sweep method");
grid on;
